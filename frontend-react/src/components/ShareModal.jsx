...
function buildShareTargets(recipe, url) {
  ...
  return [
    { name: "WhatsApp", icon: ICONS.whatsapp, color: "#25D366", href: `https://wa.me/?text=${encodedText}%20${encodedUrl}` },
    { name: "X", icon: ICONS.x, color: "#000000", href: `https://twitter.com/intent/tweet?text=${encodedText}&url=${encodedUrl}` },
    { name: "Facebook", icon: ICONS.facebook, color: "#1877F2", href: `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}` },
    { name: "Email", icon: ICONS.email, color: "#6b6157", href: `mailto:?subject=${encodedText}&body=${encodedUrl}` },
  ];
}
      ...
      <div className="share-icons">
        {buildShareTargets(recipe, url).map((s) => (
          <a key={s.name} href={s.href} target="_blank" rel="noopener noreferrer" className="share-icon-btn">
            <span className="share-icon-circle" style={{ background: s.color }}>{s.icon}</span>
            {s.name}
          </a>
        ))}
      </div>
