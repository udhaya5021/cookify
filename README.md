...
A full-stack recipe-sharing web application: user accounts secured with authenticator-app 2FA, a searchable/filterable recipe database (image *and* video), comments with NSFW filtering, ratings, saved recipes, a "Following" feed, subscriptions with email notifications, a ban-after-3-warnings system, real-time-ish chat with unread indicators, a real emailed password-reset flow, and multi-language support.
...
│       │                  chat, users
│       └── services/      security (hashing/JWT), totp_service (2FA), media (upload
│                          validation), content_filter (NSFW), email_service
└── frontend-react/        React (Vite)
    └── src/
        ├── pages/         Home, Login, Signup, ForgotPassword, ResetPassword, About,
        │                  Browse, Recipe, Upload (doubles as Edit), Profile, Messages, Chat
...
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASSWORD` / `SMTP_FROM` | Optional | Real outbound email — password reset links, comment/rating/subscription/new-recipe notifications, ban warnings. Omit entirely and emails print to the console instead, so the app runs with zero external setup. |
| `FRONTEND_URL` | Optional | Base URL used to build links in emails (e.g. the password-reset link). Defaults to `http://localhost:5173`. |
...
- **Login is authenticator-app 2FA (TOTP) only — there is no emailed code.** An emailed code travels the same channel as password reset, so a compromised inbox would defeat both factors at once; a TOTP secret never leaves the device after enrollment. An account that abandoned setup at the QR step is re-offered it at next login rather than being refused, since there's no email fallback to catch it.
- **Forgot Password sends a real single-use, 30-minute reset token by email**, not the wireframe's one-step "type a new password and go" — that version would let anyone take over any account just by knowing a username.
...
| Forgot password | Request a reset — a real emailed link with a single-use, 30-minute token |
