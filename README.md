...
- **Login uses an authenticator app (TOTP), not the emailed code the wireframe shows.** This is a deliberate deviation. An emailed code travels the same channel as password reset, so a compromised inbox defeats both factors — closer to 1.5-factor than two. Enrolment happens during signup, and an account that hasn't finished enrolling still falls back to an emailed code, because otherwise abandoning the QR step would lock the account out. Email is still doing the other five jobs the spec requires: ban warnings, comment/rating notifications to a recipe owner, new-subscriber alerts, and new-recipe alerts to subscribers.
