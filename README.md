...
- **Code formatting:** Black + isort (backend, config in `backend/pyproject.toml`), Prettier (frontend, `npm run format` / `format:check`) — run before each commit rather than left ad hoc. Linting is `oxlint` (`npm run lint`), pinned to `~1.15.0` — newer versions require Node ≥20.19 and fail silently on older Node installs.
- **Pre-login pages (Home, Login, Signup, Forgot Password) show just the logo**, no nav links — matches every wireframe for those pages, none of which include a navbar. `Layout`/`Navbar` take a `minimal` prop for this; the full navbar (Explore/Upload/Messages/Profile/language selector) is unchanged everywhere else, including Browse (which is reachable logged out but whose own wireframe was judged too sparse to use as a literal spec — see the Browse filters, below).
- **Browse keeps its full filter set** (ingredient, utensil, cost, time, calories, food type, cuisine, dietary dropdown, speed/difficulty sliders) even though its wireframe only sketches three sample controls (speed stars, difficulty stars, a veg toggle) — the Recipe Search pseudocode and the test table both require the fuller set, so the wireframe was treated as illustrative rather than exhaustive.
