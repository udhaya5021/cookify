...
- **Login uses an authenticator app (TOTP), not the emailed code the wireframe shows.** A deliberate deviation: an emailed code travels the same channel as password reset, so a compromised inbox defeats both factors — closer to 1.5-factor than two. Emailed login codes are gone entirely; enrolment happens during signup, and an account that abandoned that step is offered the QR again at its next login rather than being refused (there is no email fallback left to catch it). Email is still doing the five jobs the spec requires that have nothing to do with login: ban warnings, comment and rating alerts to a recipe owner, new-subscriber alerts, and new-recipe alerts to subscribers.
